...
def _load_tickers(path: Path) -> list[str]:
    ...
    df = pd.read_csv(path)
    # Accept either a single column or any col named 'ticker'
...
class YFLiveProducer:
    ...
    def _fetch_latest(self) -> pd.DataFrame:
        """
        Fetch latest 1m bars for tickers. yfinance limits 1m to recent days;
        period=2d keeps it within the limit and provides prev_close.
        """
        df = yf.download(
        ...
        try:
            while True:
                ...
                for ts, row in df.iterrows():
                    # Skip already sent timestamps
                    if any(ts <= self.last_sent.get(t, pd.Timestamp.min.tz_localize(timezone.utc)) for t in self.tickers):
                        continue
